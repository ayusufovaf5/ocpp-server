...
@pytest.mark.asyncio
async def test_update_firmware_happy_path_via_rest_and_ws(ocpp_http_server, db_session) -> None:
    charge_point_id = "CP_FW"
    ...
    async with websockets.connect(
        ...
        async def station_loop() -> None:
            try:
                while True:
                    ...
                    if frame[0] == MessageType.CALL and frame[2] == "UpdateFirmware":
                        assert frame[3]["location"] == firmware_url
                        assert "retrieveDate" in frame[3]
                        await ws.send(json.dumps([MessageType.CALLRESULT, frame[1], {}]))
                        return
        ...
        async with AsyncClient(
            ...
            response = await client.post(
                ...
                json={"location": firmware_url},
            )
        assert response.status_code == 200
        assert response.json() == {"status": "success", "response": {}}
