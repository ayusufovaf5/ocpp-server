...
@pytest.mark.asyncio
async def test_trigger_message_heartbeat_happy_path_via_rest_and_ws(
    ocpp_http_server, db_session
) -> None:
    charge_point_id = "CP_TRIG"
        ...
        async def station_loop() -> None:
            try:
                while True:
                    ...
                    if frame[0] == MessageType.CALL and frame[2] == "TriggerMessage":
                        assert frame[3] == {"requestedMessage": "Heartbeat"}
                        await ws.send(
