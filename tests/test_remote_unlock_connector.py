...
@pytest.mark.asyncio
async def test_unlock_connector_happy_path_via_rest_and_ws(ocpp_http_server, db_session) -> None:
    charge_point_id = "CP_UNLOCK"
    ...
    async with websockets.connect(
        ...
        async def station_loop() -> None:
            try:
                while True:
                    ...
                    if frame[0] == MessageType.CALL and frame[2] == "UnlockConnector":
                        assert frame[3] == {"connectorId": 1}
                        await ws.send(
                            json.dumps([MessageType.CALLRESULT, frame[1], {"status": "Unlocked"}])
                        )
                        return
        ...
        async with AsyncClient(
            ...
            response = await client.post(
                f"/unlock-connector/{charge_point_id}",
                json={"connector_id": 1},
            )
